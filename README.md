...
| `test_evaluation.py` | Benchmark files and frozen splits, retrieval metrics, citation scoring, the judge, the report |
...
On this repository graph expansion roughly doubles MRR on call-chain
questions. Treat that with suspicion: the questions were written by the person
who built the graph, about the code it was built on. The third-party benchmark
below is the fair test, and there the effect is smaller and mixed. Use this
table as a regression check, not as evidence.


### Third-party benchmark

The table above asks questions about CodeLens itself, the same code its
heuristics were tuned on. `scripts/eval_benchmark.py` repeats the measurement
on code CodeLens has never seen, and grades the final answers as well as the
ranking.

- **Repositories.** httpx 0.28.1, Flask 3.1.3 and Rich 15.0.0, each pinned to
  an exact commit in `scripts/benchmarks/repos.json`.
- **Questions.** 90, 30 per repository: 51 *lookup* and 39 *call-chain*. Each
  lists the files that answer it and carries a one- or two-sentence reference
  answer. Every expected file and every call-chain answer was checked against
  the pinned source with `grep`, not against CodeLens's own output.
- **Held-out split.** A hash of the question id puts one question in three into
  `test` (32 questions). Tuning may look at `dev` only; the tables report both,
  `test` first. The loader rejects a question whose split differs from the
  hash, so a hard question cannot quietly move to `dev`.
- **Retrieval.** Rank of the first expected file: Recall@K and MRR for vector,
  keyword, hybrid and hybrid+graph.
- **Answers.** Each question is answered the way `codelens ask` answers it,
  once with hybrid and once with hybrid+graph context. Two independent signals:
  - *Correctness*: an LLM judge compares the answer with the reference and says
    correct (1), partial (0.5) or incorrect (0). It never sees which
    configuration produced the answer.
  - *Citation precision*: the share of `path:line` citations in the raw answer,
    before any repair, that `CitationVerifier` confirms against the index. No
    model is involved.

```bash
.venv/bin/python scripts/eval_benchmark.py                    # retrieval only, no API calls
.venv/bin/python scripts/eval_benchmark.py --answers          # + answers, judge, citations
.venv/bin/python scripts/eval_benchmark.py --answers --update-readme
```

The first run clones the three repositories into `.bench/` and indexes each
into its own store; later runs reuse both. Answers and verdicts are cached in
`.bench/.cache/`, so a rerun only pays for questions whose context changed.
Per-question results, including every answer and the judge's reason, are
written to `scripts/benchmarks/results/latest.json`.

Read the numbers with their limits in mind. The judge is a language model and
can be wrong; when it is from the same family as the answering model it may
favour its style. The held-out split is small, so differences of a few points
are noise. One person wrote the questions and the references.

What the held-out retrieval numbers say: on third-party code graph expansion
helps recall more than it helps the top rank. It pulls in call-chain answers
that hybrid search misses entirely, and it sometimes pushes a correct first hit
down to second place, which MRR punishes. `codelens ask` gives the model the
top five chunks, so Recall@5 decides what the model gets to read. That is why
graph expansion stays on by default (`GRAPH_EXPANSION` in `config.py`). The
answer table below tests whether the extra recall turns into better answers.
The next retrieval step is to stop a neighbour from outranking the hit that
led to it, tuned on `dev` and checked once on `test`.
...
- A third-party benchmark on httpx, Flask and Rich, with a held-out split and graded answers
